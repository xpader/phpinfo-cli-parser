...
<title>PHP <?=PHP_VERSION?> - phpinfo()</title><meta name="ROBOTS" content="NOINDEX,NOFOLLOW,NOARCHIVE" /></head>
<body>
<div class="center">
    ...
    foreach ($data as $row) {
        if ($row[0] == 'value') {
            ...
            if ($row[1] == 'Directive') {
    ?>
        <tr class="h"><th><?=$row[1]?></th><th><?=$row[2]?></th><?php if (isset($row[3])) { ?><th><?=$row[3]?></th><?php } ?></tr>
    <?php } else { ?>
        <tr><td class="e"><?=$row[1]?></td><td class="v"><?=$row[2]?></td><?php if (isset($row[3])) { ?><td class="v"><?=$row[3]?></td><?php } ?></tr>
    <?php
            }
        } else {
            if ($last[0] == 'value') {
    ...
    <?php
                if (is_array($row[1])) {
                    ...
                    if (strpos($row[1], 'Zend Scripting Language Engine') !== false) {
